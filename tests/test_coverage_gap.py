...
from vector_topic_modeling.ingestion import (
    ...
    _opt_text,
    _first_non_empty,
    _materialize_column_value_fields,
...
def test_materialize_edge_cases():
    ...
    assert _materialize_column_value_fields(row={"cols": None}, config=config) == {
        "cols": None
    }

    assert _materialize_column_value_fields(row={"cols": ["bad"]}, config=config) == {
        "cols": ["bad"]
    }

    assert _materialize_column_value_fields(
        row={"cols": [{"column": " "}]}, config=config
    ) == {"cols": [{"column": " "}]}
...
def test_pick_session_main_digest_selector_success():
    ...
    rows = [
        ...
        {"session_id": "s1", "digest_hex": "d2"},
    ]
    assert pick_session_main_digest(rows, selector=good_selector) == "d2"


def test_build_digest_counts_all_pairs_bad_types():
    # Note: build_digest_counts_all_pairs intentionally treats bools as ints (True -> 1).
    # This differs from aggregate_session_topic_counts which deliberately coerces bools to 0.
    rows = [
...
def test_aggregate_session_topic_counts_edge_cases():
    # Note: aggregate_session_topic_counts intentionally coerces bools to 0 to avoid invalid aggregations.
    # This differs from build_digest_counts_all_pairs which treats True as 1.
    ...
    rows = [
        ...
        {"session_id": "s1", "digest_hex": "unknown"},  # missing topic
        {
            ...
            "digest_hex": "d2",
            "count": True,
        },  # count 0 because of bool handling in this func
