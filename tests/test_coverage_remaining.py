...
def test_cli_errors(monkeypatch):
    ...
    with patch("vector_topic_modeling.cli.build_parser") as mock_parser:
        mock_args = argparse.Namespace(command="invalid", base_url="test", api_key="test")
    ...
    with patch("vector_topic_modeling.cli.build_parser") as mock_parser:
        ...
        mock_parser.return_value.parse_args.return_value = mock_args
        with pytest.raises(ValueError, match="cluster requires --base-url and --api-key"):
            main(["cluster"])

def test_pipeline_unassigned(monkeypatch):
    ...
    assert pipeline._resolve_assignment_topic_id(
        row=rows[1],
